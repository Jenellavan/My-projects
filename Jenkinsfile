pipeline {
  ...
  environment {
    ...
    DEPLOY_PATH    = '/var/lib/tomcat10/webapps'  // <-- FIXED
    TOMCAT_SERVICE = 'tomcat10'                   // <-- FIXED
    VERSION = '1.0.0'
  ...
  stages {
    ...
    stage('Check Maven Version') {
      steps { sh 'export PATH=$MAVEN_HOME/bin:$PATH && mvn -version' }
    }

    stage('Build') {
      steps { sh 'export PATH=$MAVEN_HOME/bin:$PATH && mvn clean compile' }
    }

    stage('Unit Test') {
      steps { sh 'export PATH=$MAVEN_HOME/bin:$PATH && mvn test' }
    }

    stage('Checkstyle Analysis') {
      steps { sh 'export PATH=$MAVEN_HOME/bin:$PATH && mvn checkstyle:check' }
    }
